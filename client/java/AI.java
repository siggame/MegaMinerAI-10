    ...
    for(int ii=0;ii<creatures.length;ii++)
    {
      if (creatures[ii].getOwner() == playerID())
      {
        ...
        if ((plant == null || plant.getSize()==0) && getCreatureAtLocation(creatures[ii].getX()+1,creatures[ii].getY()) == null)
        {
          ...
          {
            if (creatures[ii].getEnergyLeft() > healthPerMove() && creatures[ii].getMovementLeft()>0)
            {
        ...
        {
          if (creatures[ii].getCurrentHealth()>=healthPerBreed() && creat.getEnergyLeft()>=healthPerBreed())
          {

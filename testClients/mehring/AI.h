...
struct Tile
{
	explicit Tile(Mappable* p = nullptr) : pObj(p), pNext(nullptr) {}
...
private:
	...
	Tile& TileAt(const VECTOR2D& pos);
...
unsigned int AI::FindPath(const VECTOR2D& from, const VECTOR2D& to, const T& heuristic, int iMaxPathLength, std::vector<VECTOR2D>& pathOut)
{
	...
	priority_queue<Tile*,std::vector<Tile*>,TileSorter> fringe;
	...
	while((pCurrentTile->pos != to) && !fringe.empty() && (pCurrentTile->G < iMaxPathLength))
	{
	    for(unsigned int i = 0; i < 4; ++i)
		{
			...
			{
				Tile& newTile = TileAt(newPos);
				
				// If the tile is walkable, 
				// todo: clean this up
				// todo: create functor to change the behavior of seeing if this is a valid node to add
				bool bLookAtTile = newTile.pObj == nullptr || (newTile.pObj->GetType() == Mappable::Creature && ((Creature*)newTile.pObj)->owner() != playerID());
